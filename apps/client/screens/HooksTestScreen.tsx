...
import { Alert, ScrollView, Button, View, ActivityIndicator, Text } from 'react-native';
...
export function HooksTestScreen() {
  ...
  return (
    <ScrollView style={{ padding: 20 }}>
      <Text style={{ fontSize: 24, fontWeight: "bold", marginBottom: 16 }}>useUpdates() 테스트 (SDK 52)</Text>
      ...
      <Section title="상태 / 오류">
        ...
        {checkError && <Status label={`체크 오류: ${checkError.message}`} err />}
        {downloadError && (
...
function Status({ label, err }: { label: string; err?: boolean }) {
  return (
    <Text style={{ marginTop: 4, color: err ? "red" : "gray" }}>
      {label}
    </Text>
  );
}
