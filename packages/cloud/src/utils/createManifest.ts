...
import type { Asset, AssetMetadata, ExpoMetadata, Manifest, Platform, PlatformMetadata, StorageClient } from "@/core";
...
const createAssets = async ({
	...
	const generateAsset = async (asset: AssetMetadata): Promise<Asset> => {
		...
		const url = await storageClient.getFileUrl({
...
const createLaunchAsset = async ({
	...
	const launchAssetFileUrl = await storageClient.getFileUrl({
